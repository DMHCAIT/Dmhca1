...
export function Header() {
  ...
  useEffect(() => {
    console.log(
      "[Header] Modal state changed - showSignupModal:",
      showSignupModal,
      "showLoginModal:",
      showLoginModal,
    );
  ...
  useEffect(() => {
    const handler = (e: StorageEvent) => {
      if (e.key === "dmhca_show_auth" && e.newValue) {
        ...
        try {
          localStorage.removeItem("dmhca_show_auth");
        } catch (err) {}
      }
    };
          ...
          <nav className="hidden lg:flex items-center gap-7 text-base">
            {nav.map((n) => (
              <Link
                key={n.to}
                to={n.to}
                className="text-foreground/80 dark:text-slate-400 hover:text-navy-deep dark:hover:text-gold transition"
                activeProps={{ className: "text-navy-deep dark:text-gold" }}
            ...
            <div className="container-x py-3 flex flex-col gap-2 text-sm md:text-base">
              {nav.map((n) => (
                <Link
                  ...
                  className="py-2 px-2 text-foreground dark:text-slate-300 hover:text-navy-deep dark:hover:text-gold hover:bg-gray-100 dark:hover:bg-slate-800 rounded-lg transition"
                >
                  {n.label}
                </Link>
              ))}
