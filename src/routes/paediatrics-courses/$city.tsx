...
export const Route = createFileRoute("/paediatrics-courses/$city")({
  component: PaediatricsCoursesPage,
});

function PaediatricsCoursesPage() {
  const { city } = useParams({
    from: "/paediatrics-courses/$city",
  });

  const specialty = "pediatrics";
  const slug = `paediatrics-courses/${city}`;

  const relatedCourses = getCoursesBySpecialty(specialty);

  const categoryObj = categories.find((cat) => cat.slug === "pediatrics");
        ...
        <div className="container-city">
          <div className="w-full">
            ...
            <h1 className="font-display text-4xl md:text-5xl text-white mb-4">
              Paediatrics Courses in {city.charAt(0).toUpperCase() + city.slice(1)}
            </h1>
          </div>
        </div>
      </div>
        ...
        <div className="w-full">
          ...
          <div className="mb-12">
            <p className="text-lg text-slate-700 leading-relaxed mb-4">
              ...
              paediatrics specialization. Our comprehensive paediatrics programs provide world-class
          ...
          <div className="space-y-4">
            <CollapsibleFAQ
              question={`Where to apply for paediatrics courses in ${city.charAt(0).toUpperCase() + city.slice(1)}?`}
              shortAnswer={`You can go with DMHCA Institute in ${city.charAt(0).toUpperCase() + city.slice(1)} for your paediatrics course because of their medical experts.`}
            />

            <CollapsibleFAQ
              question={`What is the fees of paediatrics courses in ${city.charAt(0).toUpperCase() + city.slice(1)}?`}
              shortAnswer={`The fees of paediatrics courses in ${city.charAt(0).toUpperCase() + city.slice(1)} varies from ₹50,000 to ₹3,00,000. For latest prices, contact the DMHCA medical experts.`}
            />

            <CollapsibleFAQ
              question={`How to apply for paediatrics courses in ${city.charAt(0).toUpperCase() + city.slice(1)}?`}
          ...
          <div className="mt-16 bg-gradient-to-r from-slate-900 to-slate-800 rounded-2xl p-12 text-center text-white">
            ...
            <p className="text-lg mb-8 max-w-2xl mx-auto">
              Enroll in our paediatrics program in {city.charAt(0).toUpperCase() + city.slice(1)}{" "}
          ...
          <div className="mt-16">
            <h2 className="text-3xl font-bold text-slate-900 mb-8">
              Paediatrics Courses in Other Cities
            </h2>
