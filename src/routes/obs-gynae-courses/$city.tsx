...
export const Route = createFileRoute("/obs-gynae-courses/$city")({
  component: ObsGynaeCityPage,
});

function ObsGynaeCityPage() {
  const { city } = useParams({
    from: "/obs-gynae-courses/$city",
  });

  const specialty = "obstetrics and gynaecology";
  const slug = `obs-gynae-courses/${city}`;
  ...
  const cityCourses = categorySlug
    ? courses.filter((c) => c.categories.includes(categorySlug))
    : [];
          ...
          <div className="w-full">
            ...
            <h1 className="font-display text-4xl md:text-5xl text-white mb-4">
              {specialty
                ...
                .join(" ")}{" "}
              Courses in {city.charAt(0).toUpperCase() + city.slice(1)}
            </h1>
          </div>
        </div>
      </div>
        ...
        <div className="w-full">
          ...
          <div className="mb-12">
            <p className="text-lg text-slate-700 leading-relaxed mb-4">
              ...
              obstetrics and gynaecology specialization. Our comprehensive obstetrics and
              gynaecology programs provide world-class education and practical training at DMHCA
              Institute.
            </p>
          ...
          <div className="space-y-4">
            <CollapsibleFAQ
              question={`Where to apply for obstetrics and gynaecology courses in ${city.charAt(0).toUpperCase() + city.slice(1)}?`}
              shortAnswer={`You can go with DMHCA Institute in ${city.charAt(0).toUpperCase() + city.slice(1)} for your obstetrics and gynaecology course because of their medical experts.`}
              longAnswer={`DMHCA Institute offers premium obstetrics and gynaecology programs with expert faculty and practical training.`}
            />

            <CollapsibleFAQ
              question={`What is the fees of obstetrics and gynaecology course in ${city.charAt(0).toUpperCase() + city.slice(1)}?`}
              shortAnswer={`The fees of obstetrics and gynaecology courses in ${city.charAt(0).toUpperCase() + city.slice(1)} varies from ₹50,000 to ₹3,00,000. For latest prices, contact the DMHCA medical experts.`}
              longAnswer={`Fees vary by program type and duration. Contact admissions for exact figures and payment plans.`}
            />

            <CollapsibleFAQ
              question={`How to apply for obstetrics and gynaecology course in ${city.charAt(0).toUpperCase() + city.slice(1)}?`}
          ...
          <div className="mt-16 bg-gradient-to-r from-slate-900 to-slate-800 rounded-2xl p-12 text-center text-white">
            ...
            <p className="text-lg mb-8 max-w-2xl mx-auto">
              Enroll in our obstetrics and gynaecology program in{" "}
