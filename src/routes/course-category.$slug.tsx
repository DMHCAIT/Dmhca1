  ...
  head: ({ loaderData }) => ({
    meta: loaderData
      ? [
          ...
          { property: "og:title", content: `${loaderData.category.name} Courses — DMHCA` },
          ...
          {
            name: "canonical",
            content: `https://dmhca.in/course-category/${loaderData.category.slug}`,
          },
        ]
      : [],
    links: loaderData
      ? [{ rel: "canonical", href: `https://dmhca.in/course-category/${loaderData.category.slug}` }]
      : [],
  }),
  notFoundComponent: () => (
...
function CategoryPage() {
  ...
  // CollectionPage Schema for category listing
  const collectionSchema = {
    "@context": "https://schema.org",
    "@type": "CollectionPage",
    name: `${category.name} Courses`,
    description: category.tagline,
    url: `https://dmhca.in/course-category/${category.slug}`,
    mainEntity: {
      "@type": "ItemList",
      itemListElement: list.map((course, idx) => ({
        "@type": "ListItem",
        position: idx + 1,
      ...
      <section className="container-x py-14 grid lg:grid-cols-4 gap-10">
        ...
        <div className="lg:col-span-3">
          <div className="text-xs text-muted-foreground mb-5">
            {list.length} program{list.length === 1 ? "" : "s"} in {category.name}
          </div>
          {list.length === 0 ? (
            <div className="py-12 text-muted-foreground">
              New programs are being added in this specialty.{" "}
              <Link to="/contact-us" className="text-navy-deep underline">
                Get notified
              </Link>
              .
            </div>
