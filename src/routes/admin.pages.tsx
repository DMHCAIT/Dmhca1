...
function PagesManagement() {
  ...
  const handleDelete = async (pageName: string) => {
    ...
    try {
      const { error } = await supabaseClient.from("site_pages").delete().eq("page_name", pageName);
      ...
      {editingPage && (
        <div className="bg-white rounded-lg shadow-md p-6 mb-6 border-2 border-gold">
          <div className="flex justify-between items-center mb-4">
            <h2 className="text-xl font-bold">✏️ Edit: {formData.display_name}</h2>
      ...
      {loading ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {AVAILABLE_PAGES.map((availablePage) => {
            const existingPage = pages.find((p) => p.page_name === availablePage.page_name);
            return (
              <div
                key={availablePage.page_name}
                className={`rounded-lg shadow-md p-4 border-2 ${
                  existingPage ? "border-green-300 bg-green-50" : "border-gray-300 bg-gray-100"
                }`}
              >
                ...
                <p className="text-sm text-gray-600 mb-3">
                  <code className="bg-gray-200 px-2 py-1 rounded">{availablePage.page_name}</code>
                </p>
                  ...
                  <button
                    onClick={() =>
                      handleEditPage(
                        existingPage ||
                          ({
                            page_name: availablePage.page_name,
                            display_name: availablePage.display_name,
                          } as any),
                      )
                    }
