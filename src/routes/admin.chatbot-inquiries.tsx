...
function AdminChatbotInquiries() {
  ...
  const updateNotes = async (id: string) => {
    ...
    try {
      ...
      setInquiries(inquiries.map((inq) => (inq.id === id ? { ...inq, notes } : inq)));
      ...
      <div className="mb-6 flex justify-between items-center">
        <div>
          ...
          <p className="text-sm text-gray-600 dark:text-gray-400 mt-1">
            Manage customer inquiries from the chatbot widget
          </p>
        </div>
        <div className="bg-white dark:bg-slate-800 px-4 py-2 rounded-lg shadow">
          <p className="text-sm font-semibold text-gray-700 dark:text-gray-300">
            Total: <span className="text-gold text-lg">{inquiries.length}</span>
          </p>
        </div>
      </div>
                ...
                {filteredInquiries.map((inq) => (
                  ...
                  >
                    <div className="flex justify-between items-start mb-2">
                      ...
                      <span
                        className={`px-3 py-1 rounded-full text-xs font-semibold ${getStatusColor(inq.status)}`}
                      >
                        {inq.status}
                      </span>
                    </div>
          ...
          <div className="bg-white dark:bg-slate-800 rounded-lg shadow-md p-6">
            ...
            <div className="mb-6">
              ...
              <select
                ...
                onChange={(e) =>
                  updateStatus(selectedInquiry.id, e.target.value as ChatbotInquiry["status"])
                }
            ...
            <div className="space-y-4 mb-6 pb-6 border-b dark:border-slate-700">
              ...
              <div className="bg-gray-50 dark:bg-slate-700 p-3 rounded-lg">
                ...
                <p className="text-sm font-medium text-blue-600 dark:text-white break-all">
