...
export const Route = createFileRoute("/critical-care-medicine-courses/$city")({
  component: CriticalCareMedicineCityPage,
});

function CriticalCareMedicineCityPage() {
  const { city } = useParams({
    from: "/critical-care-medicine-courses/$city",
  });

  const specialty = "critical care medicine";
      ...
      <div className="container-city py-16">
        <div className="w-full">
          ...
          <div className="space-y-4">
            <CollapsibleFAQ
              question={`Where to apply for critical care medicine courses in ${city.charAt(0).toUpperCase() + city.slice(1)}?`}
              shortAnswer={`You can go with DMHCA Institute in ${city.charAt(0).toUpperCase() + city.slice(1)} for your critical care medicine course because of their medical experts.`}
              longAnswer={`DMHCA Institute offers premium critical care medicine programs with expert faculty and practical training.`}
            />

            <CollapsibleFAQ
              question={`What is the fees of critical care medicine course in ${city.charAt(0).toUpperCase() + city.slice(1)}?`}
              shortAnswer={`The fees of critical care medicine courses in ${city.charAt(0).toUpperCase() + city.slice(1)} varies from ₹50,000 to ₹3,00,000. For latest prices, contact the DMHCA medical experts.`}
              longAnswer={`Fees vary by program type and duration. Contact admissions for exact figures and payment plans.`}
            />

            <CollapsibleFAQ
              question={`How to apply for critical care medicine course in ${city.charAt(0).toUpperCase() + city.slice(1)}?`}
          ...
          <div className="mt-16">
            <h2 className="text-3xl font-bold text-slate-900 mb-8">
              Critical Care Medicine Courses in Other Cities
            </h2>
