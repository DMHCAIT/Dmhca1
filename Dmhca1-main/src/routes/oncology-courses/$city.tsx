...
export const Route = createFileRoute("/oncology-courses/$city")({
  component: OncologyCityPage,
});

function OncologyCityPage() {
  const { city } = useParams({
    from: "/oncology-courses/$city",
  });
      ...
      <div className="container-city py-16">
        <div className="w-full">
          ...
          <div className="space-y-4">
            <CollapsibleFAQ
              question={`Where to apply for oncology courses in ${city.charAt(0).toUpperCase() + city.slice(1)}?`}
              shortAnswer={`You can go with DMHCA Institute in ${city.charAt(0).toUpperCase() + city.slice(1)} for your oncology course because of their medical experts.`}
              longAnswer={`DMHCA Institute offers premium oncology programs with expert faculty and practical training.`}
            />

            <CollapsibleFAQ
              question={`What is the fees of oncology course in ${city.charAt(0).toUpperCase() + city.slice(1)}?`}
              shortAnswer={`The fees of oncology courses in ${city.charAt(0).toUpperCase() + city.slice(1)} varies from ₹50,000 to ₹3,00,000. For latest prices, contact the DMHCA medical experts.`}
              longAnswer={`Fees vary by program type and duration. Contact admissions for exact figures and payment plans.`}
            />

            <CollapsibleFAQ
              question={`How to apply for oncology course in ${city.charAt(0).toUpperCase() + city.slice(1)}?`}
          ...
          <div className="mt-16">
            <h2 className="text-3xl font-bold text-slate-900 mb-8">
              Oncology Courses in Other Cities
            </h2>
